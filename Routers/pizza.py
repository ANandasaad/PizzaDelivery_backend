...
from Schemas.pizza import PizzaOptionResponse,PizzaOptionCreate,PizzaOptionUpdate,PizzaOptionListResponse,FilterParams
...
async def get_pizza_options(db:db_dependency,filter:Annotated[FilterParams, Depends()]):
    return await getAllPizzaOptions(db=db,filter=filter)
